...
func TestLoadingCacheWithPurge(t *testing.T) {
	...
	lc, err := NewLoadingCache(
		o.PurgeEvery(time.Millisecond*100),
		o.TTL(150*time.Millisecond),
		o.OnEvicted(func(key string, value string) { evicted = append(evicted, key, value) }),
	)
	assert.NoError(t, err)
	defer lc.Close()
...
func TestLoadingCacheWithPurgeEnforcedBySize(t *testing.T) {
	o := NewOpts[string]()
	lc, err := NewLoadingCache(o.MaxKeys(10))
	assert.NoError(t, err)
	defer lc.Close()
...
func TestLoadingCacheWithPurgeMax(t *testing.T) {
	o := NewOpts[string]()
	lc, err := NewLoadingCache(o.PurgeEvery(time.Millisecond*50), o.MaxKeys(2))
	assert.NoError(t, err)
	defer lc.Close()
...
func TestLoadingCacheInvalidateAndEvict(t *testing.T) {
	var evicted int
	o := NewOpts[string]()
	lc, err := NewLoadingCache(o.OnEvicted(func(_ string, _ string) { evicted++ }))
	assert.NoError(t, err)
	defer lc.Close()
...
func TestLoadingCacheBadOption(t *testing.T) {
	lc, err := NewLoadingCache(func(_ *LoadingCache[string]) error {
...
func TestLoadingExpired(t *testing.T) {
	o := NewOpts[string]()
	lc, err := NewLoadingCache(o.TTL(time.Millisecond * 5))
	assert.NoError(t, err)
	defer lc.Close()
