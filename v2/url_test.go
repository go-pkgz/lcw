...
func TestUrl_optionsFromQuery(t *testing.T) {
	...
	for i, tt := range tbl {
		...
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			r, err := optionsFromQuery[string](u.Query())
			if tt.fail {
				require.Error(t, err)
				return
			}
...
func TestUrl_NewLru(t *testing.T) {
	...
	res, err := New[string](u)
	require.NoError(t, err)
	r, ok := res.(*LruCache[string])
	require.True(t, ok)
...
func TestUrl_NewExpirable(t *testing.T) {
	...
	require.NoError(t, err)
	defer res.Close()
	r, ok := res.(*ExpirableCache[string])
	require.True(t, ok)
	assert.Equal(t, 10, r.maxKeys)
...
func TestUrl_NewRedis(t *testing.T) {
	...
	_, err = New[string]("redis://localhost:xxx?db=1")
	require.Error(t, err)
